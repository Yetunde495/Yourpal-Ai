...
export const MultiDropdown: React.FC<SearchMultiDropdownProps> = ({
  ...
  //   if (clearData) {
  //     setSelectedOptions([]);
  //     setClearData && setClearData(false);
  //   }
  ...
  return (
    <div ref={dropdownRef} className={`relative z-${showMenu ? "30" : "0"} `}>
      ...
      {showMenu && (
        <ul className="border-stroke dark:border-strokedark absolute left-0 top-20 z-999 mt-3 max-h-[400px] w-full overflow-y-auto rounded-md border bg-white py-2 shadow-lg dark:bg-black">
          {filteredOptions.length > 0 ? (
            filteredOptions.map((option, index) => (
              <li
                key={index}
      ...
      <div className="flex flex-wrap gap-2 mt-2">
        {selectedOptions.map((option, index) => (
          <div
            key={index}
            className="flex items-center text-sm bg-slate-300 dark:bg-[#595959] dark:text-white rounded-md p-2 mr-2"
          >
            <span>{option.label}</span>
            <button
              type="button"
              className="ml-1.5 cursor-pointer dark:bg-meta-1/60 dark:hover:bg-meta-1 dark:text-white rounded-sm px-1 text-meta-1 shadow-1 hover:bg-meta-1/25 hover:text-[#ff0000e6]"
...
export const DropdownSelect: React.FC<DropdownSelectProps> = ({
    ...
    return (
      <div ref={dropdownRef} className="relative">
        ...
        <div className="relative">
          <button
            ...
            className={`w-full appearance-none rounded border 
                       border-stroke px-5 py-3 text-left 
          ...
          >
            ...
            <div className="flex items-center justify-between">
               ...
               <span className="mr-4"><BiLoaderCircle className="text-primary animate-spin" /></span>
        ...
        {showMenu && (
          <ul className="border-stroke dark:border-strokedark absolute left-0 top-full z-50 mt-2 w-full rounded-md border bg-white py-2 shadow-lg dark:bg-black">
            {options.length > 0 ? (
              options.map((option, index) => (
                <li
                  key={index}
                  onClick={() => handleOptionClick(option)}
                  className={`hover:bg-slate-50 cursor-pointer px-4 py-2 flex justify-between items-center ${
