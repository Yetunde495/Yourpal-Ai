...
const Popover: React.FC<PopoverProps> = ({ title, children, position, icon, onClick }) => {
  ...
  return (
    ...
    >
      <span className='cursor-pointer' onClick={onClick}>{icon}</span>
      {showPopover && (
        <div className={`absolute z-10 ${positionClasses[position]} bg-white dark:bg-black min-w-65 rounded shadow-lg`}>
